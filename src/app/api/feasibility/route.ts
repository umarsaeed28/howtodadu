...
import type { SitePlanData, SitePlanBuilding, SitePlanTree, SitePlanStreet, SitePlanDriveway, SitePlanAdjacentParcel } from "@/lib/feasibility";
...
async function queryParcelWithGeometry(
  ...
  // Fallback: buffer around the point, pick by address match then proximity
  const buf = 0.0004;
...
async function queryFactors(lat: number, lng: number) {
  // Try exact point first, fall back to small buffer
  const pointParams = new URLSearchParams({
    geometry: `${lng},${lat}`,
    geometryType: "esriGeometryPoint",
    inSR: "4326",
...
async function queryBuildings(
  ...
  try {
    ...
    for (const f of data.features) {
      ...
      const cx = rings.reduce((s: number, c: number[]) => s + c[0], 0) / rings.length;
      const cy = rings.reduce((s: number, c: number[]) => s + c[1], 0) / rings.length;
  ...
  try {
    ...
    for (const f of data.features) {
      ...
      if (geom?.rings?.[0]) {
        ...
        cx = rings.reduce((s: number, c: number[]) => s + c[0], 0) / rings.length;
        cy = rings.reduce((s: number, c: number[]) => s + c[1], 0) / rings.length;
...
async function queryStreets(bbox: [number, number, number, number]): Promise<SitePlanStreet[]> {
  const [xmin, ymin, xmax, ymax] = bbox;
  const pad = 0.0002;
  const env = `${xmin - pad},${ymin - pad},${xmax + pad},${ymax + pad}`;
  const params = new URLSearchParams({
    ...
    outFields: "ORD_STNAME_CONCAT,ORD_PRE_DIR,ORD_STREET_NAME,ORD_STREET_TYPE,ORD_SUF_DIR",
    outSR: "4326",
    returnGeometry: "true",
  ...
  try {
    ...
    for (const f of data.features) {
      ...
      const parts = [str(f.attributes?.ORD_PRE_DIR), str(f.attributes?.ORD_STREET_NAME), str(f.attributes?.ORD_STREET_TYPE), str(f.attributes?.ORD_SUF_DIR)].filter(Boolean).join(" ");
...
export async function GET(request: NextRequest) {
  ...
  const [parcelResult, factors] = await Promise.all([
    queryParcelWithGeometry(coords.lat, coords.lng, address),
  ...
  const [contours, buildings, trees, streets, driveways, adjacentParcels] = await Promise.all([
    ...
    lotData && parcelRings ? queryAdjacentParcels(lotData.bbox, parcelRings, subjectPin) : [],
  ]);

  const sitePlan: SitePlanData | null =
    lotData &&
    (buildings.length > 0 || trees.length > 0 || streets.length > 0 || driveways.length > 0 || adjacentParcels.length > 0)
